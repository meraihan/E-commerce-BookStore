...
public class cart {
    ...
    //this method will genarate the prices of product names which have an id
    public void listItemsOfCart() throws SQLException, ClassNotFoundException {
        ...
        for (int i = 0; i < id.size(); i++) {
            ...
            String getItemName = "SELECT  `b_name` ,  `cat_name` ,  `price` FROM  `book` WHERE  `b_id` =" + getItemId + "";
            /*
            ...
            rs.next();
            String p_name = rs.getString("b_name");
            Double p_price = rs.getDouble("price");
            String p_Category = rs.getString("cat_name");
    ...
    public String getProductName(int id) throws SQLException, ClassNotFoundException {
        ...
        //return p_name;
        c = new db_Connection().getConnection();
        String getProductName = "SELECT  `b_name` FROM  `book` WHERE  `b_id` =" + id + "";
        Statement st = c.createStatement();
    ...
    public String getProductCategory(int id) throws SQLException, ClassNotFoundException {
        ...
        //return p_name;
        c = new db_Connection().getConnection();
        String getCategoryName = "SELECT   `cat_name`  FROM  `book` WHERE  `b_id` =" + id + "";
        Statement st = c.createStatement();
    ...
    public double getProductPrice(int id) throws SQLException, ClassNotFoundException {
        c = new db_Connection().getConnection();
        String getProductName = "SELECT  `price` FROM  `book` WHERE  `b_id` =" + id + "";
        Statement st = c.createStatement();
        ResultSet rs = st.executeQuery(getProductName);
        rs.next();
    ...
    public double getTotalPriceOfCart() throws SQLException, ClassNotFoundException {
        ...
        Statement st = c.createStatement();
        for (int i = 0; i < id.size(); i++) {
            String sqlGetPrice = "SELECT  `price` FROM  `book` WHERE  `b_id` =" + id.get(i).toString() + ";";
    ...
    public boolean addProduct(int id) throws SQLException, ClassNotFoundException {
        ...
        String sqlGetValidIds = "SELECT  `b_id` FROM  `book`;";
        ResultSet rs = st.executeQuery(sqlGetValidIds);

        while (rs.next()) {
            int id_db = rs.getInt("b_id");
            if (id_db == id) {
